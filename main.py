...
def make_scrollable(parent, bg=None):
    ...
    # Re-bind after inner is populated
    def rebind(event=None):
        _bind_scroll(inner, _scroll)
    inner.bind("<Configure>", lambda e: (canvas.configure(scrollregion=canvas.bbox("all")), rebind()))
...
def do_update(download_url, progress_cb=None):
    """Download zip, extract it, then launch the _update_apply.bat from the zip."""
    tmp_zip  = BASE_DIR / "_update.zip"
    tmp_dir  = BASE_DIR / "_update_tmp"
    bat_path = BASE_DIR / "_update_apply.bat"
    try:
        # ── Download ──────────────────────────────────────────────────────────
        ...
        if progress_cb: progress_cb(0, 1, "Preparing installer...")

        # ── Extract _update_apply.bat from zip (it's pre-built in the zip) ───
        with zipfile.ZipFile(tmp_zip, "r") as z:
            # Find _update_apply.bat anywhere in the zip
            bat_entries = [n for n in z.namelist() if n.endswith("_update_apply.bat")]
            if bat_entries:
                data = z.read(bat_entries[0])
                bat_path.write_bytes(data)

        # If bat not in zip (old zip), write a fallback bat
        if not bat_path.exists():
            bat_path.write_text(
                "@echo off\ntimeout /t 3 /nobreak >nul\n"
                f"powershell -ExecutionPolicy Bypass -Command \"Expand-Archive -LiteralPath \'{str(tmp_zip)}\' -DestinationPath \'{str(tmp_dir)}\' -Force\"\n"
                f"robocopy \"{str(tmp_dir)}\\*\" \"{str(BASE_DIR)}\" /E /XD data /NFL /NDL\n"
                f"rmdir /s /q \"{str(tmp_dir)}\" 2>nul\n"
                f"del /f /q \"{str(tmp_zip)}\" 2>nul\n"
                f"for %%f in (\"{str(BASE_DIR)}\\*.exe\") do start \"\" \"%%f\"\n"
                "del \"%~f0\"\n",
                encoding="utf-8")

        # ── Launch bat via os.startfile (avoids UAC/Defender issues) ─────────
...
class App(ctk.CTk):
    ...
    def _fill_act_tree(self):
        ...
        for d in self.dung_data.get("dungeons",[]):
            ...
            if base not in seen:
                seen[base]=tv.insert(dn,"end",iid=f"db_{base}",text=f"   ▸  {base}",open=False)
    ...
    COLS=[
        ...
        ("junk",     "Junk",       "ForgottenShippingCrate1.png",      72),
    ...
    def _draw_row(self,idx,s):
        ...
        vals=[
            ...
            (stype,               165, True),   # Type — wraplength to prevent overflow
            (dt,                  115, False),
    ...
    def _build_howto(self):
        ...
        def bul(parent,items):
            for item in items:
                ...
                lbl.pack(side="left",fill="x"); lbl.bind("<MouseWheel>",scroll_cb)
        ...
        for lbl_text,content,ico in [
            ...
            if "END Script" in lbl_text:
                ...
                warn.bind("<MouseWheel>",scroll_cb)
