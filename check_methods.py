...
def check_guild_methods(src, classes, module_funcs):
    ...
    # Toutes les méthodes de toutes les classes (aplati)
    all_class_methods = {}  # name -> [(class, lineno)]
    for cls, methods in classes.items():
        for name, lineno in methods:
            all_class_methods.setdefault(name, []).append((cls, lineno))
    ...
    for m in GUILD_METHODS_CLASS:
        ...
        if not entries:
            ...
            err(f"DOUBLON : {m} — {len(entries)} fois → " +
                ", ".join(f"{c}:L{l}" for c, l in entries))
            failures += 1
        else:
            cls, lineno = entries[0]
            ok(f"{m:<40}  {cls}:L{lineno}")

    # Fonctions module-level attendues
    module_func_names = {n: l for n, l in module_funcs}
    for f in GUILD_FUNCTIONS_MODULE:
        if f not in module_func_names:
            err(f"ABSENT (module-level) : {f}")
            failures += 1
        else:
            ok(f"{f:<40}  module:L{module_func_names[f]}")
